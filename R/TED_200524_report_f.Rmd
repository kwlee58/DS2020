...
source("./TED_data.R")
ted0524<- TED_data(data = "../data/TED_DS200524.xlsx", 
                   nrow = 110, 
...
pander(table(ted0524$group, ted0524$submit))
pander(chisq.test(table(ted0524$group, ted0524$submit),
...
```{r, echo = FALSE}
# ted0524$hours
summary(ted0524$hours)
stem(ted0524$hours)
max(ted0524$hours, na.rm = TRUE)
```
...
pander(table(cut(ted0524$days, breaks = c(0:14), include.lowest = TRUE)), caption = "일 단위")
barplot(rev(table(cut(ted0524$days, breaks = c(0:14), include.lowest = TRUE))), 
        space = 0,
...
pander(table(cut(ted0524$hours, breaks = breaks3, include.lowest = TRUE)), caption = "시간 단위")
```
...
# freqs <- table(cut(ted0524$hours, breaks = breaks2, include.lowest = TRUE))
freqs2 <- table(cut(ted0524$hours, breaks = breaks2, include.lowest = TRUE))
freqs3 <- table(cut(ted0524$hours, breaks = breaks3, include.lowest = TRUE))
freqs4 <- table(cut(ted0524$hours, breaks = breaks4, include.lowest = TRUE))
...
```{r, echo = FALSE}
pander(table(ted0524$group, ted0524$bird))
pander(chisq.test(table(ted0524$group, ted0524$bird)))
```

# 댓글의 양 

```{r, echo = FALSE}
stem(ted0524$n_chars, scale = 0.5)
```
...
```{r, echo = FALSE}
pander(tapply(ted0524$n_chars, ted0524$group, summary))
```
...
```{r, echo = FALSE}
par(family = "HCR Dotum LVT")
plot(n_chars ~ group, data = ted0524, 
     main = "그룹별 댓글의 양 비교", 
...
```{r, echo = FALSE}
pander(t.test(n_chars ~ group, data = ted0524))
```

## 마감일 기준 댓글의 양 비교

```{r, echo = FALSE}
pander(tapply(ted0524$n_chars, ted0524$bird, summary))
```
...
```{r, echo = FALSE}
par(family = "HCR Dotum LVT")
plot(n_chars ~ bird, data = ted0524, 
...
```{r, echo = FALSE}
pander(t.test(n_chars ~ bird, data = ted0524))
```
...
```{r, echo = FALSE}
# na.omit(ted0524)$name[na.omit(ted0524)$n_chars < 400]
na.omit(ted0524)$n_chars[na.omit(ted0524)$n_chars < 400]
ted0524 %>% 
  na.omit %$%
...
# na.omit(ted0524)$name[na.omit(ted0524)$n_chars < 300]
na.omit(ted0524)$n_chars[na.omit(ted0524)$n_chars < 300]
ted0524 %>% 
  na.omit %$%
...
```{r, echo = FALSE}
# na.omit(ted0524)$name[na.omit(ted0524)$n_chars < 200]
na.omit(ted0524)$n_chars[na.omit(ted0524)$n_chars < 200]
```

## Save

```{r, echo = FALSE}
saveRDS(ted0524, file = "TED200524.RDS")
```