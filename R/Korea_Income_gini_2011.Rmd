---
title: "Korea Income Distribution 2011"
author: "coop711"
date: "`r Sys.Date()`"
...
knitr::include_graphics("../pics/Labor_Income_2011.png")
```

```{r, data, highlight = FALSE}
income_kr_data <- 
  read_excel("../data/Labor_Income_2011.xlsx", 
             range = "A1:E14", 
             col_names = FALSE)
income_kr_data %<>% 
  data.frame
...
```{r}
income_kr_2 <- income_kr[1:12, ]
income_kr_2[12, ] <- apply(income_kr[12:14, ], 
                           MARGIN = 2, 
                           FUN = sum)
income_kr_2
rownames(income_kr_2)
rownames(income_kr_2)[12] <- "300 -  "
income_kr_2
(income_breaks_2 <- income_breaks[1:13])
...
```{r}
income_kr_3 <- income_kr_2[1:10, ]
income_kr_3[10, ] <- apply(income_kr_2[10:12, ], 2, sum)
rownames(income_kr_3)[8] <- "100 -   "
income_breaks_3 <- income_breaks_2[-(12:13)]
...
income_class_cum[c(1, 15)] <- c("~ 0", "0 ~ 2000")
...
title_4 <- "Lorenz Curve of Korea Wage Earners' Income 2011"
...
axis(side = 2, 
     at = income[c(1:9, 13)], 
     labels = format(income[c(1:9, 13)], nsmall = 1), 
     las = 1)
abline(h = c(0, 100), lty = 3)
abline(v = c(0, 100), lty = 3)
title(main = title_4, 
...
(c5 <- c4 + 
  geom_polygon(data = poly_df[-(12:15), ], 
               mapping = aes(x = x, y = y), 
               alpha = 0.5, fill = "grey")) 
(c6 <- c5 +
...
```{r, fig.width = 6, fig.height = 6}
c12
ggsave("../pics/cumulative_plot_wage_kr_2011.png", width = 8, height = 8)
```
...
(g8 <- g7 +
  scale_x_continuous(breaks = earners[c(1:10, 15)], 
                     labels = format(earners[c(1:10, 15)], nsmall = 1))) 
(g9 <- g8 +
  scale_y_continuous(breaks = income[c(1:12, 15)], 
                     labels = format(income[c(1:12, 15)], digits = 2, nsmall = 1)))
...
g12
ggsave("../pics/lorenz_curve_wage_kr_2011.png", width = 8, height = 8)
```
