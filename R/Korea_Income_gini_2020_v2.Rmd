---
title: "Korea Income Distribution 2020"
author: "coop711"
date: "`r Sys.Date()`"
...
knitr::include_graphics("../pics/Labor_Income_2020.png")
```

```{r, data, highlight = FALSE}
income_kr_data <- 
  read_excel("../data/Labor_Income_2020.xlsx", 
             range = "A1:E11", 
             col_names = FALSE)
income_kr_data %<>% 
  data.frame
...
```{r}
income_kr_2 <- income_kr[1:9, ]
income_kr_2[9, ] <- apply(income_kr[9:11, ], 
                           MARGIN = 2, 
                           FUN = sum)
income_kr_2
rownames(income_kr_2)
rownames(income_kr_2)[9] <- "300 -  "
income_kr_2
(income_breaks_2 <- income_breaks[1:10])
...
```{r}
income_kr_3 <- income_kr_2[1:7, ]
income_kr_3[7, ] <- apply(income_kr_2[7:9, ], 2, sum)
rownames(income_kr_3)[7] <- "100 -   "
income_breaks_3 <- income_breaks_2[-(8:9)]
...
income_class_cum[c(1, 12)] <- c("~ 0", "0 ~ 2000")
...
```{r, median}
med <- 10 + 10 * (50 - income_kr_cum[2, 1]) / (income_kr_cum[3, 1] - income_kr_cum[2, 1])
round(med, digits = 3)
```
...
(c5 <- c4 + 
  geom_polygon(data = poly_df[-(9:12), ], 
               mapping = aes(x = x, y = y), 
               alpha = 0.5, fill = "grey")) 
(c6 <- c5 +
...
(g8 <- g7 +
  scale_x_continuous(breaks = earners[c(1:7, 12)], 
                     labels = format(earners[c(1:7, 12)], nsmall = 1))) 
(g9 <- g8 +
  scale_y_continuous(breaks = income[c(1:9, 12)], 
                     labels = format(income[c(1:9, 12)], digits = 2, nsmall = 1)))
...
(g11 <- g10 +
  annotate("text", x = 80, y = 20, 
           label = paste(format(N, digits = 2, nsmall = 1), "Million"), 
           size = 9, color = "blue"))
(g12 <- g11 +
  theme_bw() +
    theme(plot.title = element_text(hjust = 0.5, size = 15),
...
g12
ggsave("../pics/lorenz_curve_wage_kr_2020.png", width = 8, height = 8)
```
