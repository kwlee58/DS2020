...
source("./TED_data.R")
ted0531<- TED_data(data = "../data/TED_DS200531.xlsx", 
                   nrow = 112, 
...
pander(table(ted0531$group, ted0531$submit))
pander(chisq.test(table(ted0531$group, ted0531$submit),
...
```{r, echo = FALSE}
# ted0531$hours
summary(ted0531$hours)
stem(ted0531$hours)
max(ted0531$hours, na.rm = TRUE)
```
...
pander(table(cut(ted0531$days, breaks = c(0:14), include.lowest = TRUE)), caption = "일 단위")
barplot(rev(table(cut(ted0531$days, breaks = c(0:14), include.lowest = TRUE))), 
        space = 0,
...
pander(table(cut(ted0531$hours, breaks = breaks3, include.lowest = TRUE)), caption = "시간 단위")
```
...
# freqs <- table(cut(ted0531$hours, breaks = breaks2, include.lowest = TRUE))
freqs2 <- table(cut(ted0531$hours, breaks = breaks2, include.lowest = TRUE))
freqs3 <- table(cut(ted0531$hours, breaks = breaks3, include.lowest = TRUE))
freqs4 <- table(cut(ted0531$hours, breaks = breaks4, include.lowest = TRUE))
...
```{r, echo = FALSE}
pander(table(ted0531$group, ted0531$bird))
pander(chisq.test(table(ted0531$group, ted0531$bird)))
```

# 댓글의 양 

```{r, echo = FALSE}
stem(ted0531$n_chars, scale = 0.5)
```
...
```{r, echo = FALSE}
pander(tapply(ted0531$n_chars, ted0531$group, summary))
```
...
```{r, echo = FALSE}
par(family = "HCR Dotum LVT")
plot(n_chars ~ group, data = ted0531, 
     main = "그룹별 댓글의 양 비교", 
...
```{r, echo = FALSE}
pander(t.test(n_chars ~ group, data = ted0531))
```

## 마감일 기준 댓글의 양 비교

```{r, echo = FALSE}
pander(tapply(ted0531$n_chars, ted0531$bird, summary))
```
...
```{r, echo = FALSE}
par(family = "HCR Dotum LVT")
plot(n_chars ~ bird, data = ted0531, 
...
```{r, echo = FALSE}
pander(t.test(n_chars ~ bird, data = ted0531))
```
...
```{r, echo = FALSE}
# na.omit(ted0531)$name[na.omit(ted0531)$n_chars < 400]
na.omit(ted0531)$n_chars[na.omit(ted0531)$n_chars < 400]
ted0531 %>% 
  na.omit %$%
...
# na.omit(ted0531)$name[na.omit(ted0531)$n_chars < 300]
na.omit(ted0531)$n_chars[na.omit(ted0531)$n_chars < 300]
ted0531 %>% 
  na.omit %$%
...
```{r, echo = FALSE}
# na.omit(ted0531)$name[na.omit(ted0531)$n_chars < 200]
na.omit(ted0531)$n_chars[na.omit(ted0531)$n_chars < 200]
```

## Save

```{r, echo = FALSE}
saveRDS(ted0531, file = "TED200531.RDS")
```