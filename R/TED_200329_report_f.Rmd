...
source("./TED_data.R")
ted0329<- TED_data(data = "../data/TED_DS200329.xlsx", 
                   nrow = 102, 
...
pander(table(ted0329$group, ted0329$submit))
pander(chisq.test(table(ted0329$group, ted0329$submit)))
```
...
```{r, echo = FALSE}
# ted0329$hours
summary(ted0329$hours)
stem(ted0329$hours)
max(ted0329$hours, na.rm = TRUE)
```
...
pander(table(cut(ted0329$days, breaks = c(0:14), include.lowest = TRUE)), caption = "일 단위")
barplot(rev(table(cut(ted0329$days, breaks = c(0:14), include.lowest = TRUE))), 
        space = 0,
...
pander(table(cut(ted0329$hours, breaks = breaks3, include.lowest = TRUE)), caption = "시간 단위")
```
...
# freqs <- table(cut(ted0329$hours, breaks = breaks2, include.lowest = TRUE))
freqs2 <- table(cut(ted0329$hours, breaks = breaks2, include.lowest = TRUE))
freqs3 <- table(cut(ted0329$hours, breaks = breaks3, include.lowest = TRUE))
# freqs4 <- table(cut(ted0329$hours, breaks = breaks4, include.lowest = TRUE))
...
```{r, echo = FALSE}
pander(table(ted0329$group, ted0329$bird))
pander(chisq.test(table(ted0329$group, ted0329$bird)))
```

# 댓글의 양 

```{r, echo = FALSE}
stem(ted0329$n_chars, scale = 0.5)
```
...
```{r, echo = FALSE}
pander(tapply(ted0329$n_chars, ted0329$group, summary))
```
...
```{r, echo = FALSE}
par(family = "HCR Dotum LVT")
plot(n_chars ~ group, data = ted0329, 
     main = "그룹별 댓글의 양 비교", 
...
```{r, echo = FALSE}
pander(t.test(n_chars ~ group, data = ted0329))
```

## 마감일 기준 댓글의 양 비교

```{r, echo = FALSE}
pander(tapply(ted0329$n_chars, ted0329$bird, summary))
```
...
```{r, echo = FALSE}
par(family = "HCR Dotum LVT")
plot(n_chars ~ bird, data = ted0329, 
...
```{r, echo = FALSE}
pander(t.test(n_chars ~ bird, data = ted0329))
```
...
```{r, echo = FALSE}
# na.omit(ted0329)$name[na.omit(ted0329)$n_chars < 400]
na.omit(ted0329)$n_chars[na.omit(ted0329)$n_chars < 400]
ted0329 %>% 
  na.omit %$%
...
# na.omit(ted0329)$name[na.omit(ted0329)$n_chars < 300]
na.omit(ted0329)$n_chars[na.omit(ted0329)$n_chars < 300]
ted0329 %>% 
  na.omit %$%
...
```{r, echo = FALSE}
# na.omit(ted0329)$name[na.omit(ted0329)$n_chars < 200]
na.omit(ted0329)$n_chars[na.omit(ted0329)$n_chars < 200]
```

## Save

```{r, echo = FALSE}
saveRDS(ted0329, file = "TED200329.RDS")
```