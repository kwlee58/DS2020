...
knitr::include_graphics("../pics/korea_labor_income_2010.png")
```
...
계산된 지니계수를 그림 안에 텍스트로 넣어주려면 `paste()`를 이용하여 입력한다.
...
```{r, fig.width = 6, fig.height = 6, fig.keep = 'all', warning = TRUE}
library(ggplot2)
(c1 <- ggplot() +
...
(c9 <- c8 +
    annotate("segment", x = 0, xend = 18.2, y = 50, yend = 50, colour = "red", linewidth = 1))
(c10 <- c9 +
...
```{r, fig.width = 6, fig.height = 6, fig.keep = 'all', warning = FALSE}
(g1 <- ggplot() +
