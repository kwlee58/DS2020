...
We need vote, class, region as `factor`s. If you leave them as `chr`, it will be coerced to factor when you tabulate it according to alphabetical order, which is not what you want. So, use `factor()` to convert them. First, make a working copy vesion of `sejong_ref`

```{r, duplicate}
sejong_ref_2 <- sejong_ref
```

```{r, vote as factor}
sejong_ref_2$vote <- factor(sejong_ref_2$vote, 
                             levels = c("yes","no"), 
                             labels = c("Yes", "No"))
```
...
sejong_ref_2$class <- factor(sejong_ref_2$class, levels = class_levels, labels = class_labels)
```
...
sejong_ref_2$region <- factor(sejong_ref_2$region, levels = region_levels, labels = region_labels)
```

```{r, data in dataframe}
str(sejong_ref_2)
kable(sejong_ref_2[4:1])
```
...
sejong_ref_2$class_2 <- factor(ifelse(sejong_ref_2$class == "Commons", 
                                       "Commons", "Bureaus"), 
                                levels = c("Bureaus", "Commons"))
kable(sejong_ref_2[c(4, 3, 5, 2, 1)])
str(sejong_ref_2)
```
...
class_2 <- sejong_ref_2$class_2
vote_region_bureaus <- xtabs(counts ~ vote + region, 
                             data = sejong_ref_2, 
                             class_2 == "Bureaus", 
                             drop = TRUE)
kable(vote_region_bureaus, caption = "Votes(Bureaus)")
# xtabs(counts ~ vote + region, data = sejong_ref_2[class_2 == "Bureaus", ], drop = TRUE)
vote_region_commons <- xtabs(counts ~ vote + region, data = sejong_ref_2, class_2 == "Commons", drop = TRUE)
...
```{r, Seoul}
region <- sejong_ref_2$region
vote_seoul_class <- xtabs(counts ~ vote + class, data = sejong_ref_2, region == "SL", drop = TRUE)
...
```{r, Chungcheong}
vote_chung_class <- xtabs(counts ~ vote + class, data = sejong_ref_2, region == "CC", drop = TRUE)
