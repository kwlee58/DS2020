...
source("./TED_data.R")
ted0614<- TED_data(data = "../data/TED_DS200614.xlsx", 
                   nrow = 108, 
...
pander(table(ted0614$group, ted0614$submit))
pander(chisq.test(table(ted0614$group, ted0614$submit),
...
```{r, echo = FALSE}
# ted0614$hours
summary(ted0614$hours)
stem(ted0614$hours)
max(ted0614$hours, na.rm = TRUE)
```
...
pander(table(cut(ted0614$days, breaks = c(0:14), include.lowest = TRUE)), caption = "일 단위")
barplot(rev(table(cut(ted0614$days, breaks = c(0:14), include.lowest = TRUE))), 
        space = 0,
...
pander(table(cut(ted0614$hours, breaks = breaks3, include.lowest = TRUE)), caption = "시간 단위")
```
...
# freqs <- table(cut(ted0614$hours, breaks = breaks2, include.lowest = TRUE))
freqs2 <- table(cut(ted0614$hours, breaks = breaks2, include.lowest = TRUE))
freqs3 <- table(cut(ted0614$hours, breaks = breaks3, include.lowest = TRUE))
freqs4 <- table(cut(ted0614$hours, breaks = breaks4, include.lowest = TRUE))
...
```{r, echo = FALSE}
pander(table(ted0614$group, ted0614$bird))
pander(chisq.test(table(ted0614$group, ted0614$bird)))
```

# 댓글의 양 

```{r, echo = FALSE}
stem(ted0614$n_chars, scale = 0.5)
```
...
```{r, echo = FALSE}
pander(tapply(ted0614$n_chars, ted0614$group, summary))
```
...
```{r, echo = FALSE}
par(family = "HCR Dotum LVT")
plot(n_chars ~ group, data = ted0614, 
     main = "그룹별 댓글의 양 비교", 
...
```{r, echo = FALSE}
pander(t.test(n_chars ~ group, data = ted0614))
```

## 마감일 기준 댓글의 양 비교

```{r, echo = FALSE}
pander(tapply(ted0614$n_chars, ted0614$bird, summary))
```
...
```{r, echo = FALSE}
par(family = "HCR Dotum LVT")
plot(n_chars ~ bird, data = ted0614, 
...
```{r, echo = FALSE}
pander(t.test(n_chars ~ bird, data = ted0614))
```
...
```{r, echo = FALSE}
# na.omit(ted0614)$name[na.omit(ted0614)$n_chars < 400]
na.omit(ted0614)$n_chars[na.omit(ted0614)$n_chars < 400]
ted0614 %>% 
  na.omit %$%
...
# na.omit(ted0614)$name[na.omit(ted0614)$n_chars < 300]
na.omit(ted0614)$n_chars[na.omit(ted0614)$n_chars < 300]
ted0614 %>% 
  na.omit %$%
...
```{r, echo = FALSE}
# na.omit(ted0614)$name[na.omit(ted0614)$n_chars < 200]
na.omit(ted0614)$n_chars[na.omit(ted0614)$n_chars < 200]
```

## Save

```{r, echo = FALSE}
saveRDS(ted0614, file = "TED200614.RDS")
```