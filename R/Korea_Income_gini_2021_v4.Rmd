...
# 누적분포가 처음으로 100% 에 도달하는 index — 그 이후는 음영영역에서 제외
# (반올림 때문에 정확히 100 에 못 닿는 경우를 대비해 약간의 허용오차 사용)
k_full <- which(earners_kor_cum_df$y >= 99.99)[1]
if (is.na(k_full)) k_full <- nrow(earners_kor_cum_df)
poly_df <- rbind(earners_kor_cum_df[seq_len(k_full), ], c(0, 100))
