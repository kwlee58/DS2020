...
legend("topright", inset = 0.05, legend = c("Xsum", "Chi-square(5)"), col = c("black", "red"), lty = 1)
...
R 코드를 설명에 포함하려면 `class_roll %$%`처럼 backtick(\` \`) 으로 묶어줍니다. `$`는 LaTeX 으로 수식을 나타낼 때 사용하는 특수 문자이기 때문에 R 코드에서 `%$%` 를 설명하고 싶은데 그대로 사용하면 수식으로 바뀔 수 있으니까 반드시 \`  \`으로 묶어 주세요. 