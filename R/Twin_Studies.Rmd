...
Nature1_tbl %>%
  ...
  scale_y_continuous(breaks = c(apply(t(Nature1), MARGIN = 2, cumsum)), 
                     labels = c(apply(t(Nature1), MARGIN = 2, cumsum)))
Nature1_tbl %>%
  ggplot(data = ., mapping = aes(x = Twins, y = Counts, fill = Resemblance)) +
  ...
  scale_y_continuous(name = "Cumulative Percentage", 
                     breaks = c(apply(t(Nature1_p), MARGIN = 2, cumsum)) / 100, 
                     labels = format(c(apply(t(Nature1_p), MARGIN = 2, cumsum)), 
                                     digits = 3, nsmall = 1)) +
