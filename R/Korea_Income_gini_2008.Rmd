---
title: "Korea Income Distribution 2008"
author: "coop711"
date: "`r Sys.Date()`"
...
knitr::include_graphics("../pics/Labor_Income_2008.png")
```

```{r, data, highlight = FALSE}
income_kr_data <- 
  read_excel("../data/Labor_Income_2008.xlsx", 
             range = "A1:E13", 
             col_names = FALSE)
income_kr_data %<>% 
  data.frame
...
(income_breaks <- c(income_breaks, 1000))
```
...
실제 인원은 거의 없는 것처럼 보이는 2억원 이상의 구간을 합쳐야 할 필요. 자료를 재구성하면,
...
income_class_cum[c(1, 14)] <- c("~ 0", "0 ~ 1000")
...
```{r, median}
med <- 12 + 5 * (50 - income_kr_cum[4]) / (income_kr_cum[5] - income_kr_cum[4])
round(med, digits = 3)
```
...
plot(income_kr_cum_df, 
     ...
     ann = FALSE, 
     xlim = c(0, 100))
abline(h = income_kr_cum[10, 2], 
       lty = 3, col = "red")
axis(side = 2, 
     at = income_kr_cum[10, 2], 
     label = income_kr_cum[10, 2], 
     las = 1)
title(main = title_3, 
      xlab = xlab_2, 
      ylab = ylab_3)
...
title_4 <- "Lorenz Curve of Korea Wage Earners' Income 2008"
...
axis(side = 2, 
     at = income[c(1:8, 12)], 
     labels = format(income[c(1:8, 12)], nsmall = 1), 
     las = 1)
abline(h = c(0, 100), lty = 3)
abline(v = c(0, 100), lty = 3)
title(main = title_4, 
...
(c5 <- c4 + 
  geom_polygon(data = poly_df[-(10:13), ], 
               mapping = aes(x = x, y = y), 
               alpha = 0.5, fill = "grey")) 
(c6 <- c5 +
...
```{r, fig.width = 6, fig.height = 6}
c12
ggsave("../pics/cumulative_plot_wage_kr_2008.png", width = 8, height = 8)
```
...
(g8 <- g7 +
  scale_x_continuous(breaks = earners[c(1:9, 14)], 
                     labels = format(earners[c(1:9, 14)], nsmall = 1))) 
(g9 <- g8 +
  scale_y_continuous(breaks = income[c(1:11, 14)], 
                     labels = format(income[c(1:11, 14)], digits = 2, nsmall = 1)))
...
g12
ggsave("../pics/lorenz_curve_wage_kr_2008.png", width = 8, height = 8)
```
