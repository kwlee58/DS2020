---
title: "Korea Income Distribution 2010"
author: "coop711"
date: "`r Sys.Date()`"
...
knitr::include_graphics("../pics/Labor_Income_2010.png")
```

```{r, data, highlight = FALSE}
income_kr_data <- 
  read_excel("../data/Labor_Income_2010.xlsx", 
             range = "A1:E21", 
             col_names = FALSE)
income_kr_data %<>% 
  data.frame
...
```{r}
income_kr_2 <- income_kr[1:19, ]
income_kr_2[19, ] <- apply(income_kr[19:21, ], 
                           MARGIN = 2, 
                           FUN = sum)
income_kr_2
rownames(income_kr_2)
rownames(income_kr_2)[19] <- "300 -  "
income_kr_2
(income_breaks_2 <- income_breaks[1:20])
...
```{r}
income_kr_3 <- income_kr_2[1:17, ]
income_kr_3[17, ] <- apply(income_kr_2[17:19, ], 2, sum)
rownames(income_kr_3)[15] <- "100 -   "
income_breaks_3 <- income_breaks_2[-(19:20)]
...
income_class_cum[c(1, 22)] <- c("~ 0", "0 ~ 2000")
...
여기서 계산해 보면 $\frac{x-17}{20-17} = \frac{50 - 48.6}{54.3-48.6}$로부터 $x = 17.7$가 계산된다.
급여의 평균은 급여총계 401,086,049(백만원)을 근로인원 15,176,782명으로 나누어 준 26.4백만 원이 된다.

```{r, median}
med <- 17 + 3 * (50 - income_kr_cum[7]) / (income_kr_cum[8] - income_kr_cum[7])
round(med, digits = 3)
```
...
plot(income_kr_cum_df, 
     ...
     ann = FALSE, 
     xlim = c(0, 100))
abline(h = income_kr_cum[18, 2], 
       lty = 3, col = "red")
axis(side = 2, 
     at = income_kr_cum[11, 2], 
     label = income_kr_cum[18, 2], 
     las = 1)
title(main = title_3, 
      xlab = xlab_2, 
      ylab = ylab_3)
...
title_4 <- "Lorenz Curve of Korea Wage Earners' Income 2010"
...
axis(side = 2, 
     at = income[c(1:16, 20)], 
     labels = format(income[c(1:16, 20)], nsmall = 1), 
     las = 1)
abline(h = c(0, 100), lty = 3)
abline(v = c(0, 100), lty = 3)
title(main = title_4, 
...
(c2 <- c1 +
  scale_x_continuous(breaks = earners_kor_cum_df$x[c(1, 2, 4, 6, 8:22)],
                     labels = earners_kor_cum_df$x[c(1, 2, 4, 6, 8:22)],
                     limits = c(0, 200)))
(c3 <- c2 +
...
(c5 <- c4 + 
  geom_polygon(data = poly_df[-(19:22), ], 
               mapping = aes(x = x, y = y), 
               alpha = 0.5, fill = "grey")) 
(c6 <- c5 +
...
```{r, fig.width = 6, fig.height = 6}
c12
ggsave("../pics/cumulative_plot_wage_kr_2010.png", width = 8, height = 8)
```
...
(g8 <- g7 +
  scale_x_continuous(breaks = earners[c(1:13, 22)], 
                     labels = format(earners[c(1:13, 22)], nsmall = 1))) 
(g9 <- g8 +
  scale_y_continuous(breaks = income[c(1:19, 22)], 
                     labels = format(income[c(1:19, 22)], digits = 2, nsmall = 1)))
...
g12
ggsave("../pics/lorenz_curve_wage_kr_2010.png", width = 8, height = 8)
```
