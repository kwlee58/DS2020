...
function (tbl_df) 
{
    ...
    tbl_p_df <- tbl_df %>% select(3) %>% proportions %>% data.frame(tbl_df[1:2], 
        Prop = .)
    tbl_p_df$width <- tapply(tbl_p_df[, 3], tbl_p_df[, 2], sum) %>% 
        rep(each = N)
    tbl_p_df$height <- unlist(tapply(tbl_p_df[, 3], tbl_p_df[, 
        2], proportions))
    tbl_p_df$label_height <- unlist(tapply(tbl_p_df$height, tbl_p_df[, 
        2], function(x) cumsum(x) - x/2))
    tbl_p_df$y_breaks <- unlist(tapply(tbl_p_df$height, tbl_p_df[, 
        2], cumsum))
    tbl_p_df$center <- tapply(tbl_p_df[, 3], tbl_p_df[, 2], sum) %>% 
        pos %>% rep(each = N)
    tbl_p_m <- tapply(tbl_p_df[, 3], tbl_p_df[, 2], sum)
    x_breaks <- c(0, ifelse(cumsum(tbl_p_m) < 0.10000000000000001, 
        0, cumsum(tbl_p_m)))
    ...
    m <- ggplot(tbl_p_df, aes(x = center, y = height, width = width)) + 
        ...
        scale_y_continuous(breaks = y_breaks, label = y_label) + 
