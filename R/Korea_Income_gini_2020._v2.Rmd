...
knitr::include_graphics("../pics/korea_taxable_income_2020.png")
```

```{r, data, highlight = FALSE}
income_kr_data <- 
  read.table("../data/korea_taxable_income_2020.txt", 
             header = TRUE, 
             row.names = 1)
str(income_kr_data)
income_kr_sum <- cbind(income_kr_data[, 1] + income_kr_data[, 2], income_kr_data[, 3] + income_kr_data[, 4]) 
N <- 
...
income_kr <- 
  ...
  round(2) 
str(income_kr)
dimnames(income_kr) <- list(rownames(income_kr_data), c("Earners(%)", "Income(%)"))
income_kr
```
...
```{r, fig.width = 6, fig.height = 6}
c12
ggsave("../pics/cumulative_plot_wage_kr_2020.png", width = 8, height = 8)
```
