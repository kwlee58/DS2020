...
#> x축에 생기는 공간 없애기;
  scale_y_continuous(expand = expand_scale(mult = c(0, 0.01), add = c(0, 0.01)))
#  coord_flip()
```
...
* `Gini_b_a_order_melt`의 `Country_order`가 개선도 올림차순으로 정리되어 있는 `factor`이기 때문에 그대로 활용하면 됨. 이 데이터는 `Before`가 `After`보다 항상 큰 값을 갖기 떄문에 `position = position_identity()` 를 적용하기 매우 좋은 사례임.
...
#  scale_fill_brewer(type = "qual", palette = "Set1", direction = -1) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.58)) +
  ...
  coord_flip()
ggsave("../pics/Gini_OECD.png", width = 7, height = 9)
```
